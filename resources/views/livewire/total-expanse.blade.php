<div class="col-lg-4 mt-5">
    <div class="card">
        <div class="card-header text-center bg-dark p-3">
            <h3 class="text-white">ব্যয়</h3>
            @if ($totalExpenseSum === null)
                    <small class="text-white">( {{$lastExpense->created_at->format('d-M-Y')}} থেকে শুরু )</small>
                @endif
        ...
        <div class="card-body text-center" style="background-color: #d3d3d3">
            @if ($totalExpenseSum !== null)
                <h3 class="pt-4">{{ $totalExpenseSum }}/=</h3>
            @else
                <h3 class="pt-4">{{ $expenseInYear }}/=</h3>
            @endif
        </div>
    </div>
</div>
